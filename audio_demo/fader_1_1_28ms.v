// -------------------------------------------------------------
// 
// File Name: /home/shahryar/Desktop/ECE532/TestHDLMatlab/hdlsrc/audio_setup/fader_1_1_28ms.v
// Created: 2020-04-03 17:38:39
// 
// Generated by MATLAB 9.7 and HDL Coder 3.15
// 
// -------------------------------------------------------------


// -------------------------------------------------------------
// 
// Module: fader_1_1_28ms
// Source Path: audio_setup/audio_core/Drum1/fader-1 1.28ms
// Hierarchy Level: 2
// 
// -------------------------------------------------------------

`timescale 1 ns / 1 ns

module fader_1_1_28ms
          (clk,
           reset,
           enb,
           Input_rsvd,
           reset_1,
           enb_1,
           Out1);


  input   clk;
  input   reset;
  input   enb;
  input   signed [19:0] Input_rsvd;  // sfix20_En18
  input   reset_1;
  input   enb_1;
  output  signed [19:0] Out1;  // sfix20_En18


  wire Constant1_out1;
  wire Constant3_out1;
  reg [28:0] HDL_Counter5_out1;  // ufix29
  wire Switch1_out1;
  wire HDL_Counter5_step_roll2InitialValue;  // ufix1
  reg [28:0] HDL_Counter5_stepreg;  // ufix29
  reg [28:0] HDL_Counter5_stepregneg;  // ufix29
  wire Compare_To_Zero_out1;
  wire switch_compare_1;
  wire [28:0] HDL_Counter5_count_step;  // ufix29
  wire signed [28:0] Data_Type_Conversion7_out1;  // sfix29_En27
  wire signed [19:0] Data_Type_Conversion6_out1;  // sfix20_En18
  reg signed [19:0] Input_rsvd_1;  // sfix20_En18
  wire switch_compare_1_1;
  wire signed [19:0] Constant4_out1;  // sfix20_En18
  wire signed [19:0] Switch_out1;  // sfix20_En18
  reg signed [19:0] HwModeRegister1_reg [0:6];  // sfix20 [7]
  wire signed [19:0] HwModeRegister1_reg_next [0:6];  // sfix20_En18 [7]
  wire signed [19:0] Switch_out1_1;  // sfix20_En18
  wire signed [39:0] Product1_out1;  // sfix40_En36
  reg  [0:7] delayMatch1_reg;  // ufix1 [8]
  wire [0:7] delayMatch1_reg_next;  // ufix1 [8]
  wire Compare_To_Zero_out1_1;
  wire switch_compare_1_2;
  reg signed [39:0] Product1_out1_1;  // sfix40_En36
  wire signed [19:0] Data_Type_Conversion1_out1;  // sfix20_En18
  wire signed [19:0] Constant6_out1;  // sfix20_En18
  wire signed [19:0] Switch2_out1;  // sfix20_En18


  assign Constant1_out1 = 1'b0;


  assign Constant3_out1 = 1'b0;


  // Count limited, Unsigned Counter
  //  initial value   = 134217728
  //  step value      = 1
  //  count to value  = 0
  always @(posedge clk or posedge reset)
    begin : HDL_Counter5_step_process
      if (reset == 1'b1) begin
        HDL_Counter5_stepreg <= 29'b00000000000000000000000000001;
        HDL_Counter5_stepregneg <= 29'b11111111111111111111111111111;
      end
      else begin
        if (enb) begin
          if (reset_1 == 1'b1) begin
            HDL_Counter5_stepreg <= 29'b00000000000000000000000000001;
          end
          else if (Switch1_out1 == 1'b1) begin
            if (HDL_Counter5_step_roll2InitialValue == 1'b1) begin
              HDL_Counter5_stepreg <= 29'b01000000000000000000000000000;
            end
            else begin
              HDL_Counter5_stepreg <= 29'b00000000000000000000000000001;
            end
          end
          if (reset_1 == 1'b1) begin
            HDL_Counter5_stepregneg <= 29'b11111111111111111111111111111;
          end
          else if (Switch1_out1 == 1'b1) begin
            if (HDL_Counter5_step_roll2InitialValue == 1'b1) begin
              HDL_Counter5_stepregneg <= 29'b01000000000000000000000000000;
            end
            else begin
              HDL_Counter5_stepregneg <= 29'b11111111111111111111111111111;
            end
          end
        end
      end
    end

  assign HDL_Counter5_step_roll2InitialValue = (((HDL_Counter5_out1 == 29'b11111111111111111111111111111) && (Constant3_out1 == 1'b1)) || ((HDL_Counter5_out1 == 29'b00000000000000000000000000001) && (Constant3_out1 == 1'b0)) ? 1'b1 :
              1'b0);


  assign switch_compare_1 = Compare_To_Zero_out1 > 1'b0;



  assign Switch1_out1 = (switch_compare_1 == 1'b0 ? enb_1 :
              Constant1_out1);


  always @(posedge clk or posedge reset)
    begin : HDL_Counter5_process
      if (reset == 1'b1) begin
        HDL_Counter5_out1 <= 29'b01000000000000000000000000000;
      end
      else begin
        if (enb) begin
          if (reset_1 == 1'b1) begin
            HDL_Counter5_out1 <= 29'b01000000000000000000000000000;
          end
          else if (Switch1_out1 == 1'b1) begin
            HDL_Counter5_out1 <= HDL_Counter5_out1 + HDL_Counter5_count_step;
          end
        end
      end
    end

  assign HDL_Counter5_count_step = (Constant3_out1 == 1'b1 ? HDL_Counter5_stepreg :
              HDL_Counter5_stepregneg);



  assign Data_Type_Conversion7_out1 = HDL_Counter5_out1;


  assign Data_Type_Conversion6_out1 = Data_Type_Conversion7_out1[28:9];


  assign Compare_To_Zero_out1 = Data_Type_Conversion6_out1 == 20'sb00000000000000000000;


  always @(posedge clk or posedge reset)
    begin : HwModeRegister_process
      if (reset == 1'b1) begin
        Input_rsvd_1 <= 20'sb00000000000000000000;
      end
      else begin
        if (enb) begin
          Input_rsvd_1 <= Input_rsvd;
        end
      end
    end



  assign switch_compare_1_1 = Compare_To_Zero_out1 > 1'b0;



  assign Constant4_out1 = 20'sb00000000000000000000;


  assign Switch_out1 = (switch_compare_1_1 == 1'b0 ? Data_Type_Conversion6_out1 :
              Constant4_out1);


  always @(posedge clk or posedge reset)
    begin : HwModeRegister1_process
      if (reset == 1'b1) begin
        HwModeRegister1_reg[0] <= 20'sb00000000000000000000;
        HwModeRegister1_reg[1] <= 20'sb00000000000000000000;
        HwModeRegister1_reg[2] <= 20'sb00000000000000000000;
        HwModeRegister1_reg[3] <= 20'sb00000000000000000000;
        HwModeRegister1_reg[4] <= 20'sb00000000000000000000;
        HwModeRegister1_reg[5] <= 20'sb00000000000000000000;
        HwModeRegister1_reg[6] <= 20'sb00000000000000000000;
      end
      else begin
        if (enb) begin
          HwModeRegister1_reg[0] <= HwModeRegister1_reg_next[0];
          HwModeRegister1_reg[1] <= HwModeRegister1_reg_next[1];
          HwModeRegister1_reg[2] <= HwModeRegister1_reg_next[2];
          HwModeRegister1_reg[3] <= HwModeRegister1_reg_next[3];
          HwModeRegister1_reg[4] <= HwModeRegister1_reg_next[4];
          HwModeRegister1_reg[5] <= HwModeRegister1_reg_next[5];
          HwModeRegister1_reg[6] <= HwModeRegister1_reg_next[6];
        end
      end
    end

  assign Switch_out1_1 = HwModeRegister1_reg[6];
  assign HwModeRegister1_reg_next[0] = Switch_out1;
  assign HwModeRegister1_reg_next[1] = HwModeRegister1_reg[0];
  assign HwModeRegister1_reg_next[2] = HwModeRegister1_reg[1];
  assign HwModeRegister1_reg_next[3] = HwModeRegister1_reg[2];
  assign HwModeRegister1_reg_next[4] = HwModeRegister1_reg[3];
  assign HwModeRegister1_reg_next[5] = HwModeRegister1_reg[4];
  assign HwModeRegister1_reg_next[6] = HwModeRegister1_reg[5];



  assign Product1_out1 = Input_rsvd_1 * Switch_out1_1;


  always @(posedge clk or posedge reset)
    begin : delayMatch1_process
      if (reset == 1'b1) begin
        delayMatch1_reg[0] <= 1'b0;
        delayMatch1_reg[1] <= 1'b0;
        delayMatch1_reg[2] <= 1'b0;
        delayMatch1_reg[3] <= 1'b0;
        delayMatch1_reg[4] <= 1'b0;
        delayMatch1_reg[5] <= 1'b0;
        delayMatch1_reg[6] <= 1'b0;
        delayMatch1_reg[7] <= 1'b0;
      end
      else begin
        if (enb) begin
          delayMatch1_reg[0] <= delayMatch1_reg_next[0];
          delayMatch1_reg[1] <= delayMatch1_reg_next[1];
          delayMatch1_reg[2] <= delayMatch1_reg_next[2];
          delayMatch1_reg[3] <= delayMatch1_reg_next[3];
          delayMatch1_reg[4] <= delayMatch1_reg_next[4];
          delayMatch1_reg[5] <= delayMatch1_reg_next[5];
          delayMatch1_reg[6] <= delayMatch1_reg_next[6];
          delayMatch1_reg[7] <= delayMatch1_reg_next[7];
        end
      end
    end

  assign Compare_To_Zero_out1_1 = delayMatch1_reg[7];
  assign delayMatch1_reg_next[0] = Compare_To_Zero_out1;
  assign delayMatch1_reg_next[1] = delayMatch1_reg[0];
  assign delayMatch1_reg_next[2] = delayMatch1_reg[1];
  assign delayMatch1_reg_next[3] = delayMatch1_reg[2];
  assign delayMatch1_reg_next[4] = delayMatch1_reg[3];
  assign delayMatch1_reg_next[5] = delayMatch1_reg[4];
  assign delayMatch1_reg_next[6] = delayMatch1_reg[5];
  assign delayMatch1_reg_next[7] = delayMatch1_reg[6];



  assign switch_compare_1_2 = Compare_To_Zero_out1_1 > 1'b0;



  always @(posedge clk or posedge reset)
    begin : PipelineRegister_process
      if (reset == 1'b1) begin
        Product1_out1_1 <= 40'sh0000000000;
      end
      else begin
        if (enb) begin
          Product1_out1_1 <= Product1_out1;
        end
      end
    end



  assign Data_Type_Conversion1_out1 = Product1_out1_1[37:18];


  assign Constant6_out1 = 20'sb00000000000000000000;


  assign Switch2_out1 = (switch_compare_1_2 == 1'b0 ? Data_Type_Conversion1_out1 :
              Constant6_out1);


  assign Out1 = Switch2_out1;

endmodule  // fader_1_1_28ms

